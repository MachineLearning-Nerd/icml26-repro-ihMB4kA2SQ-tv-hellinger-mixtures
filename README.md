...
| [`orx/evaluator-visible-judge-remediation-release`](https://github.com/MachineLearning-Nerd/icml26-repro-ihMB4kA2SQ-tv-hellinger-mixtures/tree/orx/evaluator-visible-judge-remediation-release) | Cumulative candidate, raw artifact promotion, visibility and red-team gates | `uv sync --frozen && uv run python repro/src/run_publication_gate.py` | Artifact-generation run `7bc34e8e…` passed at `094d92e`; publication pending final gates | local CPU, `1m35s`, estimator explicitly capped to 1 thread |
